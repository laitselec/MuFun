...
from .image_preprocess import ImagePreprocess,load_audio
...
class LazyRLDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        data_dict = self.text_preprocess(copy.deepcopy(sources["conversations"]), mode='rl')
        # data_dict['image'] = []
        image_size = []
        # if 'image' in sources:
...
class DataCollatorForRLDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        gt = [instance['gt'] for instance in instances]
        ...
        for instance in instances:
            batch['images'].extend(instance['image'])
        if batch['images']:
...
def make_rl_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                data_args) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = LazyRLDataset(tokenizer=tokenizer,
                                          data_path=data_args.data_path,
                                          data_args=data_args)
    eval_dataset = LazyRLDataset(tokenizer=tokenizer,
