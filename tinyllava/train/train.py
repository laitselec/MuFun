...
def has_checkpoint_folders_pathlib_any(parent_dir: str) -> bool:
    p = pathlib.Path(parent_dir)
    if not p.is_dir():
        return False
    # any() 会在找到第一个匹配项后立即返回 True，效率更高
    return any(
        item.is_dir() and item.name.split('-')[-1].isdigit() 
        for item in p.glob('checkpoint-*')
    )
...
def train():
    ...
    if has_checkpoint_folders_pathlib_any(training_arguments.output_dir):
        ...
        print(f"No checkpoint folders found in {training_arguments.output_dir}, starting training from scratch...")
        trainer.train()
