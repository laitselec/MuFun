...
class GRPOTrainer(Trainer):
    ...
    @profiling_decorator
    def _get_per_token_logps(self, model, input_ids, attention_mask, images, image_sizes, logits_to_keep, batch_size=None) -> torch.Tensor:
        ...
        for i in range(0, input_ids.size(0), batch_size):
            ...
            logits = model(
                input_ids=input_ids_batch, attention_mask=attention_mask_batch, images=images, image_sizes=image_sizes, logits_to_keep=logits_to_keep + 1
            ).logits
    ...
    def _generate_and_score_completions(
        ...
        if 'images' in inputs:
            inputs.pop('images')
        inputs = dict_of_lists_to_list_of_dicts(inputs)
            ...
            with unwrap_model_for_generation(
                ...
                with (
                    ...
                    prompt_completion_ids = unwrapped_model.generate(
                        prompt_ids, attention_mask=prompt_mask, images=images, image_sizes=image_sizes, generation_config=self.generation_config
                    )
        ...
        with torch.no_grad():
            ...
            if self.num_iterations > 1 or self.args.steps_per_generation > self.args.gradient_accumulation_steps:
                old_per_token_logps = self._get_per_token_logps(
                    self.model, prompt_completion_ids, attention_mask, images, image_sizes, logits_to_keep, batch_size
                )
            else:
    ...
    def _compute_loss(self, model, inputs):
        # Compute the per-token log probabilities for the model
        prompt_ids, prompt_mask = inputs["prompt_ids"], inputs["prompt_mask"]
        images, image_sizes = inputs["images"], inputs["image_sizes"]
        completion_ids, completion_mask = inputs["completion_ids"], inputs["completion_mask"]
        input_ids = torch.cat([prompt_ids, completion_ids], dim=1)
        attention_mask = torch.cat([prompt_mask, completion_mask], dim=1)
        logits_to_keep = completion_ids.size(1)  # we only need to compute the logits for the completion tokens
        # print(inputs)
        per_token_logps = self._get_per_token_logps(model, input_ids, attention_mask, images, image_sizes, logits_to_keep)
        ...
        if self.beta != 0.0:
            with torch.no_grad():
                if self.ref_model is not None:
                    ref_per_token_logps = self._get_per_token_logps(
                        self.ref_model, input_ids, attention_mask, images, image_sizes, logits_to_keep
                    )
                else:
                    with self.accelerator.unwrap_model(self.model).disable_adapter():
                        ref_per_token_logps = self._get_per_token_logps(
                            self.model, input_ids, attention_mask, images, image_sizes, logits_to_keep
                        )
            per_token_kl = (
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys: Optional[list[str]] = None):
        ...
        print(f"Prediction step loss: {loss.item()}")
        return loss, None, None

    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        mode = "train" if self.model.training else "eval"
        metrics = {key: sum(val) / len(val) for key, val in self._metrics[mode].items()}  # average the metrics
        # print(f"Logging metrics for {mode} mode: {metrics}")
